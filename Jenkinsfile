node {
    ...
    PC_TOKEN = sh(script:'''curl -s -k -H 'Content-Type: application/json' -H 'accept: application/json' --data '{\"username\":\"$PC_USER\", \"password\":\"$PC_PASS\"}' https://${AppStack}/legacy/signin | jq --raw-output .token''', returnStdout:true).trim()
    }
